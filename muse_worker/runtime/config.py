import os
from pathlib import Path

from muse_worker.paths import RUNTIME_DIR, WORKER_ROOT, resolve_packaged_path

COMFY_URL = os.environ.get("COMFY_URL", "http://127.0.0.1:8188").rstrip("/")

def _looks_like_comfy_root(path: Path) -> bool:
    return (path / "main.py").exists() or (path / "models").exists()

def _discover_comfy_root() -> Path:
    explicit = os.environ.get("COMFY_ROOT")
    if explicit:
        return Path(explicit)

    candidates = [
        WORKER_ROOT / "ComfyUI",
        Path("C:/ComfyUI_windows_portable/ComfyUI"),
        Path("/workspace/ComfyUI"),
        Path("/root/volume/ComfyUI"),
        Path("/runpod-volume/ComfyUI"),
        Path("/mnt/data/ComfyUI"),
        Path("/volume/ComfyUI"),
    ]
    for candidate in candidates:
        if _looks_like_comfy_root(candidate):
            return candidate
    return Path("/workspace/ComfyUI")

COMFY_ROOT = _discover_comfy_root()
WORKFLOW_PATH = resolve_packaged_path(os.environ.get("WORKFLOW_PATH"), RUNTIME_DIR / "workflow.json", RUNTIME_DIR)
WORKFLOW_IMG2IMG_PATH = resolve_packaged_path(
    os.environ.get("WORKFLOW_IMG2IMG_PATH"),
    RUNTIME_DIR / "workflow_img2img.json",
    RUNTIME_DIR,
)
WORKFLOW_NODE_MAP_PATH = resolve_packaged_path(os.environ.get("WORKFLOW_NODE_MAP_PATH"), RUNTIME_DIR / "workflow_map.json", RUNTIME_DIR)
PRESETS_PATH = resolve_packaged_path(os.environ.get("PRESETS_PATH"), RUNTIME_DIR / "presets.json", RUNTIME_DIR)
UI_TITLE = os.environ.get("UI_TITLE", "Comfy Vast Studio")
APP_PASSWORD = os.environ.get("APP_PASSWORD") or os.environ.get("UI_PASSWORD")

CHECKPOINT_DIR = COMFY_ROOT / "models" / "checkpoints"
LORA_DIR = COMFY_ROOT / "models" / "loras"
UPSCALE_DIR = COMFY_ROOT / "models" / "upscale_models"
DIFFUSION_MODEL_DIR = COMFY_ROOT / "models" / "diffusion_models"
UNET_DIR = COMFY_ROOT / "models" / "unet"
TEXT_ENCODER_DIR = COMFY_ROOT / "models" / "text_encoders"
VAE_DIR = COMFY_ROOT / "models" / "vae"
OUTPUT_DIR = COMFY_ROOT / "output"

IMAGE_SUFFIXES = (".png", ".jpg", ".jpeg", ".webp")
FAVORITES_PATH = resolve_packaged_path(os.environ.get("FAVORITES_PATH"), RUNTIME_DIR / "favorites.json", RUNTIME_DIR)

DEFAULT_NODE_MAP = {
    "checkpoint": "5",
    "positive": "7",
    "negative": "8",
    "sampler": "9",
    "vae_decode": "10",
    "save": "12",
    "latent": "13",
    "load_image": "14",
    "vae_encode": "15",
    "upscale_model": "27",
    "upscale": "28",
}
